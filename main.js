...
//Kendaraan
{
  ...
  const curve = new THREE.CatmullRomCurve3([
    ...
    new THREE.Vector3(-8, 5, 150),
    new THREE.Vector3(-8, 5, 180),
    new THREE.Vector3(-32, 5, 370),
    new THREE.Vector3(-32, 5, 500),
    new THREE.Vector3(-32, 5, 10000)
  ]);
  let carPosition = 0;
  let isStopped = false;
  let stopTime = 0;
  let firstPersonCamera, thirdPersonCamera, activeCamera;
    ...
    .load('car1.mtl', function (materials) {
      ...
      new OBJLoader()
        ...
        .load('car1.obj', function (object) {
          ...
          firstPersonCamera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 10000);
          firstPersonCamera.position.set(0, 4, 0); // Position the camera inside the car
          car1.add(firstPersonCamera);
  
          // Setup third person camera
          thirdPersonCamera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 10000);
          thirdPersonCamera.position.set(0, 10, -20); // Position the camera behind the car
          thirdPersonCamera.lookAt(20, 5, 0);
  
          // Set the active camera
          activeCamera = thirdPersonCamera;
  
          // Event listener for switching camera
          ...
          function animate(time) {
            const dt = (time - time_prev) * 0.001; // Convert time to seconds
            if (car1 && !isStopped) {
              carPosition += dt * 0.05; // Adjust speed here
              if (carPosition > 1) carPosition = 0; // Loop the car position along the curve
              car1.position.copy(curve.getPointAt(carPosition));
  
              // Align the car to the curve direction
              const tangent = curve.getTangentAt(carPosition).normalize();
              car1.lookAt(car1.position.clone().add(tangent));
  
              const carBox = new THREE.Box3().setFromObject(car1);
              for (const pagar of pagars) {
                const pagarBox = new THREE.Box3().setFromObject(pagar);
                if (carBox.intersectsBox(pagarBox)) {
            ...
            if (isStopped && (time - stopTime) > 10000) { // 10 seconds stop
              isStopped = false;
            }
...
// cctv
{
  let cctvLeft, cctvRight;
  let cctvLeftRotationDirection = 1;
  let cctvRightRotationDirection = 1;
    ...
    .load('cctv.mtl', function (materials) {
      ...
      new OBJLoader()
        ...
        .load('cctv.obj', function (object) {
          object.position.set(-198.4, 62, 85);
          ...
          // Light for left CCTV
          var light = new THREE.SpotLight(0xFFFFFF, 50000);
          light.position.set(-220, 95, 85);
          light.angle = 0.4;
          light.penumbra = 1;
          light.decay = 2;
          light.distance = 410;
          light.castShadow = true;
          light.target.position.set(-40, -260, 75);
          scene.add(light.target);
          scene.add(light);
          const spotLightHelper = new THREE.SpotLightHelper(light);
          scene.add(spotLightHelper);
  ...
  new MTLLoader()
    ...
    .load('cctv.mtl', function (materials) {
      ...
      new OBJLoader()
        ...
        .setPath('resources/ModelGrafkom/cctv/')
        .load('cctv.obj', function (object) {
          object.position.set(-198.4, 62, 0);
          object.scale.setScalar(0.6);
          object.traverse(function (child) {
            if (child.isMesh) {
              child.castShadow = true;
          ...
          // Light for right CCTV
          var light = new THREE.SpotLight(0xFFFFFF, 50000);
          light.position.set(-220, 95, 10);
          light.angle = 0.4;
          light.penumbra = 1;
          light.decay = 2;
          light.distance = 410;
  ...
  loadCCTVStand(-198.4, 62, 85);
  loadCCTVStand(-198.4, 62, 0);

  let timePrev = 0;
  function animateCCTV(time) {
    const dt = (time - timePrev) * 0.001;

    if (cctvLeft) {
      cctvLeft.rotation.y += cctvLeftRotationDirection * 0.5 * dt;
      if (cctvLeft.rotation.y > Math.PI / 4 || cctvLeft.rotation.y < -Math.PI / 4) {
        cctvLeftRotationDirection *= -1;
      }
    }

    if (cctvRight) {
      cctvRight.rotation.y += cctvRightRotationDirection * 0.5 * dt;
      if (cctvRight.rotation.y > Math.PI / 4 || cctvRight.rotation.y < -Math.PI / 4) {
        cctvRightRotationDirection *= -1;
      }
    }

    renderer.render(scene, camera);
    timePrev = time;
    requestAnimationFrame(animateCCTV);
  }
  requestAnimationFrame(animateCCTV);
}
